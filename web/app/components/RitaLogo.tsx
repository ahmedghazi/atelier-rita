...
const RitaLogo = ({
  ...
  // Once the scrub reaches endWidth this flips true and the logo locks
  // there permanently — the intro never plays again.
  ...
  const { startWidth, endWidth, range } = shrink ?? {};
  ...
  useEffect(() => {
    ...
    // No intro requested, or it already completed → hold at endWidth, no animation.
    if (!playIntro || lockedRef.current) {
      gsap.set(el, { width: endWidth });
      return;
    }

    let formatTimer: ReturnType<typeof setTimeout> | undefined;

    // One-shot, scroll-scrubbed shrink. Locks permanently when endWidth is reached.
    const tween = gsap.fromTo(
      ...
      { width: startWidth },
      {
        ...
        scrollTrigger: {
          ...
          scrub: true,
          onLeave: () => {
            // Reached endWidth → stay here forever, never animate again.
            lockedRef.current = true;
            tween.scrollTrigger?.kill();
            gsap.set(el, { width: endWidth });
            formatTimer = setTimeout(() => publish("FORMAT_CHANGED"), 500);
          },
        },
      },
    );
  ...
  }, [animated, playIntro, layoutReady, startWidth, endWidth, range]);
